...
router.post("/login", async (req, res) => {
  try {
    ...
    if (user.tenantId) {
      const students = await Student.find({ tenantId: user.tenantId }).select("_id");
      const teachers = await Teacher.find({ tenantId: user.tenantId }).select("_id");
      ...
      if (recipients.length) {
        await notifyEvent("LOGIN", {
          userId: user._id,
          tenantId: user.tenantId,
          fullName: `${fullName} has logged in`,
          role: "system",
